 ...
 */
public class GetUserMediaImpl extends Service {
    ...
    public class LocalBinder extends Binder {
        GetUserMediaImpl getService() {
            return GetUserMediaImpl.this;
        }
    }
    ...
    public void stopMe() {
        ...
        stateProvider.setPeerConnectionFactory(null);
    }
    ...
    void getDisplayMedia(
            final ConstraintsMap constraints, final AnyThreadResult result) {
        ConstraintsMap videoConstraintsMap = null;
        ConstraintsMap videoConstraintsMandatory = null;
        ...
        screenRequestPremissions(
                new ResultReceiver(new Handler(Looper.getMainLooper())) {
                    ...
                    protected void onReceiveResult(int requestCode, Bundle resultData) {
                        ...
                        final VideoCapturer videoCapturer = new ScreenCapturerAndroid(
                                mediaProjectionData,
                        ...
                        if (VERSION.SDK_INT >= VERSION_CODES.Q) {
                            Log.i(TAG, "Asking for audio recording permissions");
                            final ArrayList<String> permissions = new ArrayList<>();
                            permissions.add(PERMISSION_AUDIO);
                            requestPermissions(
                                    permissions, args -> {
                                        Log.i(TAG, "User allowed audio");
                                        getDisplayMedia(resultData, videoCapturer, true, result);
                                    }, args -> {
                                        Log.e(TAG, "User revoked audio permissions, audio won't be shared");
                                        getDisplayMedia(resultData, videoCapturer, false, result);
                                    });
                        } else {
                            getDisplayMedia(resultData, videoCapturer, false, result);
                        }
    ...
    private void getDisplayMedia(Bundle resultData, VideoCapturer videoCapturer, boolean hasAudio, Result result) {
        int resultCode = resultData.getInt(GRANT_RESULTS);
        Intent mediaProjectionData = resultData.getParcelable(PROJECTION_DATA);

        MediaStreamTrack[] tracks = new MediaStreamTrack[2];
        if (VERSION.SDK_INT >= VERSION_CODES.Q && hasAudio) {
            Log.i(TAG, "Asking for audio recording permissions");
            try {
                MediaProjectionManager mediaProjectionManager =
                        (MediaProjectionManager) GetUserMediaImpl.this.getSystemService(Context.MEDIA_PROJECTION_SERVICE);
                MediaProjection mediaProjection = mediaProjectionManager.getMediaProjection(resultCode, mediaProjectionData);
                ...
                AudioRecord record = new AudioRecord.Builder()
                        ...
                        .setAudioFormat(new AudioFormat.Builder()
                                ...
                                .build())
                        .build();
                Log.i(TAG, "Initialized screen audio sharing, initializing RTC");
                initializePeerConnectionFactory(record);

                Log.i(TAG, "Starting the audio recording");
                ...
                Log.i(TAG, "Audio initialized");
            } catch (Exception e) {
                Log.e(TAG, "Cannot initialize audio sharing: " + e.getMessage());
                initializePeerConnectionFactory(null);
            }
        } else {
            initializePeerConnectionFactory(null);
        }
        ...
        videoCapturer.initialize(
                surfaceTextureHelper, GetUserMediaImpl.this, videoSource.getCapturerObserver());

        WindowManager wm =
                (WindowManager) GetUserMediaImpl.this.getSystemService(Context.WINDOW_SERVICE);
        ...
        final MediaStream mediaStream = stateProvider.getPeerConnectionFactory().createLocalMediaStream(stateProvider.getNextStreamUUID());
        if (mediaStream == null) {
    ...
    void initializePeerConnectionFactory(AudioRecord audioRecord) {
        if (audioDeviceModule == null) {
            audioDeviceModule = JavaAudioDeviceModule.builder(this)
                    ...
                    .setCustomAudioRecord(audioRecord)
                    .createAudioDeviceModule();
            EglBase.Context eglContext = EglUtils.getRootEglBaseContext();
            stateProvider.setPeerConnectionFactory(PeerConnectionFactory.builder()
