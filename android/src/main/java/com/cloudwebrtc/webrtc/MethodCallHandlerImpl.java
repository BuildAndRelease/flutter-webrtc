...
public class MethodCallHandlerImpl implements MethodCallHandler, StateProvider {
  ...
  private boolean ensureInitialized(final AnyThreadResult result) {
    if (mFactory == null || getUserMediaImpl == null) {
      result.error("initialize", "The plugin is not initialized. Call `initialize` and then `getUserMedia` or `getDisplayMedia` to make it work.", null);
      return false;
    } else {
      return true;
    }
...
//      result.success(null);
    ...
    intent.putExtra(GetUserMediaImpl.NOTIFICATION_CHANNEL, (String)call.argument("channelName"));

    connection = new ServiceConnection() {
      @Override
      public void onServiceConnected(ComponentName className, IBinder service) {
        GetUserMediaImpl.LocalBinder binder = (GetUserMediaImpl.LocalBinder) service;
        getUserMediaImpl = binder.getService();
        getUserMediaImpl.stateProvider = MethodCallHandlerImpl.this;
        result.success(null);
      }
  ...
  private void deinitialize(final AnyThreadResult result) {
    if (getUserMediaImpl == null) {
      result.error("initialize", "The plugin has not been initialized.", null);
      return;
    }
  ...
  public void onMethodCall(MethodCall call, @NonNull Result notSafeResult) {
    ...
    switch (call.method) {
      ...
      case "getUserMedia": {
        ...
        getUserMediaImpl.getUserMedia(constraintsMap, result);
        return;
      }
      case "getDisplayMedia": {
        Map<String, Object> constraints = call.argument("constraints");
        ConstraintsMap constraintsMap = new ConstraintsMap(constraints);
        getUserMediaImpl.getDisplayMedia(constraintsMap, result);
        return;
      }
