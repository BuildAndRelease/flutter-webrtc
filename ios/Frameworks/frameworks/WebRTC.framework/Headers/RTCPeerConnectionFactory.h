...
/* Initialize object with speaker audio and injectable video encoder/decoder factories */
- (instancetype)
    initWithSpeakerAudioAndEncoderFactory:(nullable id<RTC_OBJC_TYPE(RTCVideoEncoderFactory)>)encoderFactory
            decoderFactory:(nullable id<RTC_OBJC_TYPE(RTCVideoDecoderFactory)>)decoderFactory;
...
- (void)sendAudioFrame:(void*)audio_data
			withBitsPerSample: (int)bits_per_sample
			andSampleRate:(int)sample_rate
			andNumberOfChannels:(int)number_of_channels
			andNumberOfFrames:(int)number_of_frames;
